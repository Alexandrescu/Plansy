...
{
    public class TomorrowSchedulesReminderJob : BaseJob
    {
        #region Constructor

        /// <summary>
        ///     Initializes a new instance of the <see cref="TomorrowSchedulesReminderJob" /> class.
        /// </summary>
        /// <param name="job"></param>
        public TomorrowSchedulesReminderJob(AutomaticProcessingJob job)
            : base(job)
        {
                ...
                using (var db = new AppDbContext())
                {
                    ...
                    var schedules =
                        ...
                        where r.State == ScheduleState.Valid &&
                              DbFunctions.TruncateTime(r.ScheduleDateTimeStart) == tomorrow &&
                    ...
                    foreach (var schedule in schedules)
                    {
                        ...
                        var notification = RetrieveOthers.AddNotification(schedule.Id, schedule.UserId,
                            ...
                            Resource.Notification_TomorrowReminder +schedule.ScheduleDateTimeStart.ToString(Thread.CurrentThread.CurrentCulture) +
                            Resource.Notification_TodayReminder2 + name + ". "+Resource.Notification_TwoHoursReminder2 + phone);
